    ...
    public function hold($id = null){
      ...
      $this->set('cart_items', $cart_item);
          ...
          $this->Flash->set('Items are reserved for two weeks', ['element' => 'success']);
            ...
            $index = $cart_item->first();
            $index1 = $cart_item->skip(1)->first();
            $index2 = $cart_item->skip(2)->first();
           ...
           /* if ($index) {
            ...
            if ($index1) {
                ...
                $index1barcode = 'Barcode: '.$index1->item_copy->barcode."\n";
                $index1call = 'Call Number: '.$index1->item_copy->item->call."\n";
                $index1title = 'Title: '.$index1->item_copy->item->title."\n";
                $index1type = 'Item Type: '.$index1->item_copy->item->item_type->type_name."\n";

                if ($index2) {
                    $index1barcode = 'Barcode: '.$index2->item_copy->barcode."\n";
                    $index1call = 'Call Number: '.$index2->item_copy->item->call."\n";
                    $index1title = 'Title: '.$index2->item_copy->item->title."\n";
                    $index1type = 'Item Type: '.$index2->item_copy->item->item_type->type_name."\n";
            ...
            $this->redirect(['action' => 'viewReserves', 'controller' => 'reserves']);
            ...
            $email->from(['ie.onefourtech@gmail.com' => 'EDV Website'])
                ->to(['matthewstobo1@gmail.com']) // edv email
                ->subject('Item Reservation')
                ->send('Dear EDV,' . "\n" . "\n" . 'The following member has reserved the following item, please remove this item from the shelf and store in the designated reserves area. Details are below:' . "\n" . "\n" . "\n" .
                    'Name: ' . $user_fname . ' ' . $user_lname . "\n" .
                    'Address: ' . $user_address . ',' . "\n" . $user_suburb . ',' . ' ' . $user_postcode . ', ' . $user_state . "\n" .
                    'Email: ' . $user_email . "\n" .
                    'Contact Number: ' . $user_number . "\n" . "\n" . "\n" . 'Item Details' . "\n"."\n"
                    .'Barcode: '. $index->item_copy->barcode."\n".'Call Number: '.$index->item_copy->item->call."\n" .'Title: '.$index->item_copy->item->title."\n" .'Item Type: '.$index->item_copy->item->item_type->type_name."\n"."\n"
                    . $index1barcode . $index1call . $index1title  . $index1type."\n"
                    . $index2barcode . $index2call . $index2title . $index2type.'       '.$date.' '.' '.$return);*/
       ...
       $apiContext = new ApiContext(new OAuthTokenCredential($ppClient->key, $ppSecret->key));        if (isset($_GET['success']) && $_GET['success'] == 'true') {
            ...
            if ($payments->save($newPayment)) {
              ...
              /*  if ($index) {
                ...
                $email->from(['ie.onefourtech@gmail.com' => 'EDV Website'])
                    ->to(['ie.expo.team14@gmail.com']) //edv email
                    ->subject('Post Items')
                    ->send('Dear EDV,' . "\n" . "\n" . 'The following member has borrowed the following items, please post to the address. Details are below:' . "\n" . "\n" . "\n" .
                        'Name: ' . $user_fname . ' ' . $user_lname . "\n" .
                        'Address: ' . $user_address . ',' . "\n" . $user_suburb . ',' . ' ' . $user_postcode . ', ' . $user_state . "\n" .
                        'Email: ' . $user_email . "\n" .
                        'Contact Number: ' . $user_number . "\n" . "\n" . "\n" .
